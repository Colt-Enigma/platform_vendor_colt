...
- coltgerrit:      A Git wrapper that fetches/pushes patch from/to ColtOS Gerrit Review.
- coltrebase:      Rebase a Gerrit change and push it again.
- coltremote:      Add git remote for ColtOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for Colt-Enigma Github.
...
function coltremote()
{
    ...
    local COLT_USER=$(git config --get review.review.coltos.org.username)
    if [ -z "$COLT_USER" ]
    then
        git remote add colt ssh://review.coltos.org:29418/$PFX$PROJECT
    else
        git remote add colt ssh://$COLT_USER@review.coltos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'colt' created"
}
...
function makerecipe() {
    ...
    then
        pwd
        coltremote
        git push colt HEAD:refs/heads/'$1'
    fi
    '
}
...
function coltgerrit() {
    ...
    local user=`git config --get review.review.coltos.org.username`
...
function coltrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "ColtOS Gerrit Rebase Usage: "
        echo "      coltrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.coltos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                colt|otapackage|bacon|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
